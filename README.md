...
Git-deliver is usable as-is, but is still a work in progress. My main objective now is to consolidate what I have, before I had any new features. Here are a some of the elements that need work:

*  Test on OS X and BSD
*  Add better error handling, and clearer help messages, decide what goes to stderr and stdout.
*  Clean up the test suite and make it more reliable
*  Clean up deliver.sh, make it more readable and maintainable
*  Can we push just what's needed when delivering to a remote and not the whole branch ?
*  See what the minimum Git version on the remote needs to be and add a test for that before any call to the remote Git
*  Write presets for common development environments
*  Run some outputs through `git var GIT_PAGER` (remote-status when reading the full history, others ?)

Some ideas for future developments include:

*  Seeing what could be done with multi-url remotes and remote groups
*  Avoiding repeating SSH connections (see branch 'single-ssh' for a crude attempt)
*  Asking for confirmation before switching versions, maybe with an option to see the complete diff through `$PAGER` since the last delivery, and an option to check this diff against the ones generated by running the same update on other environments
*  Adding an option to automatically push the delivery tag to a given remote
