...
Git-deliver is usable as-is, but is still a work in progress. Here are a some of the elements that need work:

*  Test on OS X and BSD
*  Add better error handling, and clearer help messages
*  Clean up the test suite and make it more reliable
*  Clean up deliver.sh, make it more readable and maintainable
*  See what could be done with multi-url remotes and remote groups
*  Avoid repeating SSH connections (see branch 'single-ssh' for a crude attempt)
*  Can we push just what's needed when delivering to a remote and not the whole branch ?
*  Ask for confirmation before switching versions, maybe with an option to see the complete diff through `$PAGER` since the last delivery, and an option to check this diff against the ones generated by running the same update on other environments
