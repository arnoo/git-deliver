...
function deliver
	...
	run_remote "test -L \"$REMOTE_PATH/delivered/preprevious\" && { rm \"$REMOTE_PATH/delivered/preprevious\" || exit 1 ; } ; \
		    test -L \"$REMOTE_PATH/delivered/previous\" && { mv \"$REMOTE_PATH/delivered/previous\" \"$REMOTE_PATH/delivered/preprevious\" || exit 2 } ; \
		    test -L \"$REMOTE_PATH/delivered/current\" && { cp -d \"$REMOTE_PATH/delivered/current\"  \"$REMOTE_PATH/delivered/previous\" || exit 3 } ; \
		    cd \"$REMOTE_PATH\"/delivered && { ln -sfn \""`basename "$DELIVERY_PATH"`"\" \"new\" && mv -Tf \"$REMOTE_PATH/delivered/new\" \"$REMOTE_PATH/delivered/current\" || exit 4 ; }"
